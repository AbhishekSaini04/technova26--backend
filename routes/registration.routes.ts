...
import {registerEvent, myEvents, doIRegisteredForEvent,allRegistrations, getRegistrationsByEvent} from "../controllers/registration.controller";
const router = Router();


router.post("/register/:id", authMiddleware, registerEvent);
router.get("/myEvents", authMiddleware, myEvents);
router.get("/doIRegisteredForEvent/:id", authMiddleware, doIRegisteredForEvent);
